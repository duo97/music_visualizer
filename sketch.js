...
function draw(){
  ...
  if (mySound.isPlaying()){
    ...
    let octaveBins=new Array(9);for (let i=0; i<9; i+=1) octaveBins[i] = 0;
    let octaveAve=new Array(9);
    let octaveDeviaF=new Array(9);
    let octaveBinsExg=new Array(9);for (let i=0; i<9; i+=1) octaveBinsExg[i] = 0;
    let totalSum=0
    ...
    for (let octave=0;octave<9;octave++){
      ...
      for (let note = 0; note<12; note++){ 
        ...
        let w=Math.floor(energy**Exgfactor/octaveBinsExg[octave]*width);
        let solid_w=Math.floor(w/3*2);
        rect(x, y,solid_w+2 , -h);
        let gradient_w=w-solid_w
        if (gradient_w>=1&&h>=1){
        let img=creatGradientRect( gradient_w,h,hues[note],hues[(note+1)%11],Brightness,Saturation);
