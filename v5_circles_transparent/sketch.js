...
function draw() {
  ...
  transparency1 = map(amp, 0, 0.2, 0.4, 0.6);
  transparency2 = map(amp, 0, 0.2, 0, 0.2);
  ...
  if (mySound.isPlaying()) {
    ...
    for (let octave = 1; octave < 9; octave++) {
      ...
      for (let note = 0; note < 12; note++) {
        ...
        let zoomFactor = 1.5;
        fill(hues[note], Saturation, Brightness);
        ellipse(x + w / 2, y - h / 2, w, w);
        fill(hues[note], Saturation, Brightness, transparency1);
        ellipse(x + w / 2, y - h / 2, w * zoomFactor, w * zoomFactor);
        fill(hues[note], Saturation, Brightness, transparency2);
        ellipse(x + w / 2, y - h / 2, w * zoomFactor ** 2, w * zoomFactor ** 2);
